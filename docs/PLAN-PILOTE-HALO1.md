...
1. Lire IRQ1. Sur RX_DR :
   ...
   - `lastArm_ = now`, puis renvoyer true.
2. Sinon, si OMST ≠ RX : `enterRxMode(300)`, `lastArm_ = now`.
...
- **Comptes et traces** : chaque relance L2 est annoncee par une ligne affichee meme sans trace (jamais bloquante : perdue et comptee dans « traces perdues » si le tampon serie est plein), par exemple `[lampe] BM5602 : 3 paquets de suite sans TX_DS ni MAX_RT en 30 ms : relance automatique du module (1 depuis la derniere guerison)`, puis, apres `halo.begin()`, `[lampe] BM5602 relance : quartz pret, calibration faite` (`BC5602::begin()` reussit des que la version se lit, quartz pret ou non : cette ligne dit ce qui a vraiment ete refait) ; de meme le passage EN PANNE et le retour. `lampe` montre la serie de delais, la fenetre d'ecoute en cours, l'etat EN PANNE et la derniere relance datee (lisible meme si sa ligne a ete perdue) ; `lampe stats` les relances par cause (verif., delais, bruit), les 4 dernieres datees, les relances de suite sans guerison et l'attente avant la prochaine permise. `lampe stats raz` remet les compteurs a zero, pas l'attente ni l'etat EN PANNE.
...
| 20 | Puce bloquee que la verification ne voit pas (incident du 24/09 : quartz touche, registres conformes, tous les envois en delai) | L2 sur symptome : 3 delais TX de suite, ou deluge de CRC faux en ecoute ; jamais sur une lampe muette. Phase sourde sans envoi : vue seulement a la commande suivante (C.5) |
---
